...
property myRFG : missing value -- will be created upon first running of DoFinderSelection()

to DoFinderSelection(a)
	
	try
		set scriptName to a's scriptName
	on error
		set scriptName to "Do Finder Selection"
	end try
	
	log "DFS: Beginning " & scriptName & "."
	...
	set myRFG to RFG's GetRFG("DFS", {scriptName})
	
	(* get local fixed copy; immutable if items are deleted *)
	...
	repeat with theFinderItem in theFinderSelection
		--	log (theFinderItem)
		--	tell application "Finder" to set theName to name of theFinderItem
		
		--		tell myRFG to Notify(a's actionName & " for " & itemKind & " \"" & (name of theItem) & "\"" & ".")
		
		--		tell myRFG to Notify(name of theFinderItem)
		--copy a's DoTheItem(contents of theFinderItem) to the end of r
		copy a's DoTheItem(theFinderItem) to the end of r
	end repeat
	return r
...
script a
	property scriptName : "DFS's Example Script"
	property actionName : "Showing"
	
	to DoTheItem(theItem)
		--		global myRFG
		set kindOfTheItem to (kind of theItem)
		
		tell myRFG to Notify(actionName & space & kindOfTheItem & " \"" & (name of theItem) & "\"" & ".")
...
to GetFinderSelection()
	...
	tell myRFG to Notify("Using frontmost window's target " & " \"" & (theTopFolderDisplayedName) & "\"" & " as container.")
	
	if theTopFolderName is not "" then return {theTopFolder}
